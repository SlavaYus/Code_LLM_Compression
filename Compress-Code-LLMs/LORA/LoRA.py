...
def python_code_instructions_18k_alpaca_ru():
    traindata = load_from_disk('iamtarun', 'python_code_instructions_18k_alpaca_ru', split='train')
    valdata = load_from_disk('iamtarun', 'python_code_instructions_18k_alpaca_ru', split='validation')
    return traindata, valdata
...
def apply_lora(model, tokenizer, batch_size=64, micro_batch_size=4, cutoff_len=256, add_eos_token=False,
    ...
    try:
        ...
        data = DatasetDict({'train':load_from_disk(args.data_path)})
...
if __name__ == "__main__":
    ...
    parser.add_argument('--load_data_from_local', type=bool, default=False, help='Get or not data from hugging face')
